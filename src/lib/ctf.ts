...
export const botResonse = async (validator: boolean, req: NextApiRequest, roomID: number, message: string, file: any) => {
    if (roomID === 1) {
        const paramsData = validCheckEnv(message, ['url', 'label'])
        if (paramsData.valid === false) {
            return invalidFormat
        }

        return ctf1(validator, { url: paramsData.params.url, label: paramsData.params.label })
    } else if (roomID === 2) {
        const paramsData = validCheckEnv(message, ['n'])
        if (paramsData.valid === false) {
            return invalidFormat
        }
        return ctf2(validator, { req: req, n: paramsData.params.n })
    } else if (roomID === 3) {
        const paramsData = validCheckEnv(message, ['search', 'n'])
        if (paramsData.valid === false) {
            return invalidFormat
        }
        return await ctf3(validator, { search: paramsData.params.search, n: paramsData.params.n })
    } else if (roomID === 4) {
        return await ctf4(validator, { message: message, file: file})
    }  else if (roomID === 5) {
...
const ctf1 = (validator: boolean, params: { url: string, label: string }) => {
    const url = params.url
    const label = params.label
    try {
        const _url = new URL(url)
        if (validator && _url.protocol === 'javascript:') {
            return { gotFlag: true, html: "正解！" }
        }
    } catch { }
    return {
        gotFlag: false,
        html: `<a href="${encodeURI(url)}">${escape(label)}</a>`
    }
}

const ctf2 = (validator: boolean, params: { req: NextApiRequest, n: string }) => {
    ...
    if (n < 1 || n > 100) {
        ...
        if (validator && url.search.length > 0 && (decodeURI(url.search).includes('<') || decodeURI(url.search).includes('>'))) {
            return { gotFlag: true, html: "正解！" }
        }
        return { gotFlag: false, html: `Error: n must be an integer between 1 and 100 <br>at ${decodeURI(url.href)}` }
    }
...
const ctf3 = async (validator: boolean, params: { search: string, n: string }) => {
...
const ctf4 = async (validator: boolean, params: { message: string, file: any }) => {
