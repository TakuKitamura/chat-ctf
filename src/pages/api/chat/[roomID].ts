...
async function chatRoute(req: NextApiRequest, res: NextApiResponseServerIO) {
  ...
  if (req.method === "GET") {
    ...
    await form.parse(req, async function (err: Error, fields: { message: string }, files: any) {
      const isFile = isEmpty(fields)
      if (userID === 'Admin' && isFile === false) {
        if (fields.message === '\\disable-validator') {
          validator = false
          res?.socket?.server?.io?.emit("got_flag", `${userID?.toString().toUpperCase()} disable validator`);
          res.status(200).json({ "status": "ok" });
          return
        } else if (fields.message === '\\enable-validator') {
          validator = true
          res?.socket?.server?.io?.emit("got_flag", `${userID?.toString().toUpperCase()} enable validator`);
          res.status(200).json({ "status": "ok" });
          return
        } else if (fields.message === '\\reset-chat') {
          const db = await open(
            {
              filename: './mydb.sqlite',
              driver: sqlite3.Database
          ...
          res.status(200).json({ "status": "ok" });
          return
        } else if (fields.message === '\\reload') {
          res?.socket?.server?.io?.emit("reload");
          res.status(200).json({ "status": "ok" });
          return
        } else if (fields.message.startsWith('\\eval\n')) {
          const script = fields.message.slice(6)
          res?.socket?.server?.io?.emit("eval", script);
          res.status(200).json({ "status": "ok" });
          return
        }
      }
      ...
      const result = await botResonse(validator, req, roomID, message, file)
